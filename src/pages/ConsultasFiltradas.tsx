...
interface ConsultasFiltradasProps {
  companyKey: string;
  pageTitle: string;
  subtitle: string;
  filterRegex: RegExp;
...
const ConsultasFiltradas = ({ companyKey, pageTitle, subtitle, filterRegex }: ConsultasFiltradasProps) => {
  ...
  const [authenticated, setAuthenticated] = useState<boolean>(() => {
    try {
      return sessionStorage.getItem(`verConsultasAuth-${companyKey}`) === 'true';
    } catch {
      return false;
    }
  });
  ...
  const handlePasswordSubmit = (e?: any) => {
    ...
    if (passwordInput.trim() === 'medcheck') {
      try {
        sessionStorage.setItem(`verConsultasAuth-${companyKey}`, 'true');
      } catch {
  ...
  const cargarConsultas = async () => {
    ...
    try {
      ...
      if (supabaseError) {
        ...
        const filtered = (data || []).filter((consulta: Consulta) =>
          filterRegex.test(consulta.empresa || '')
        );
  ...
  const descargarExcel = async () => {
    ...
    try {
      ...
      if (filtered.length === 0) {
        setError('No hay consultas filtradas para este mes');
        return;
      }

      const excelData = filtered.map((c: Consulta) => ({
        Nombre: c.nombre || '',
        Empresa: c.empresa || '',
        Cargo: c.cargo || '',
        Teléfono: c.telefono || '',
      ...
      const filename = `${companyKey}_Agendamientos_${monthNameText}_${year}.xlsx`;
      ...
      <div className="relative py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h1 className="text-4xl sm:text-5xl font-bold text-[#001F54] mb-4">{pageTitle}</h1>
            ...
            <div className="bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden">
              ...
              {consultas.length > 0 ? (
                          ...
                          <div className="px-6 pb-6 border-t border-gray-200 pt-4 animate-fadeIn">
                            <div className="mb-6">
                              <div className="flex items-center gap-2 mb-4">
                                ...
                                <h4 className="font-bold text-lg text-[#001F54]">Datos de quien Agenda</h4>
                              </div>
                                ...
                                <div className="space-y-3">
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm text-gray-800 font-medium">{c.empresa || 'No especificada'}</p>
                                    </div>
                                  </div>
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm text-gray-800 font-medium">{c.cargo || 'No especificado'}</p>
                                    </div>
                                  </div>
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm text-gray-800 font-medium">{c.telefono || 'No especificado'}</p>
                                    </div>
                                  </div>
                                </div>
                                <div className="space-y-3">
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm text-gray-800 font-medium break-all">{c.email || 'No especificado'}</p>
                                    </div>
                                  </div>
                                  {c.creado_en && (
                                    <div className="flex items-start gap-3">
                            ...
                            <div className="mb-6">
                              <div className="flex items-center gap-2 mb-4">
                                ...
                                <h4 className="font-bold text-lg text-[#001F54]">Datos del Colaborador Agendado</h4>
                              </div>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-3">
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm text-gray-800 font-medium">{c.nombre}</p>
                                    </div>
                                  </div>
                                  <div className="flex items-start gap-3">
                                    <Briefcase className="w-4 h-4 text-[#FF6B35] flex-shrink-0 mt-1" />
                                ...
                                <div className="space-y-3">
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-sm text-gray-800 font-medium">{c.numero_documento || 'No especificado'}</p>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            ...
                            <div className="mb-6">
                              <div className="flex items-center gap-2 mb-4">
                                ...
                                <h4 className="font-bold text-lg text-[#001F54]">Detalles de la Consulta</h4>
                              </div>
                              <div className="space-y-4">
                                <div className="flex items-start gap-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm text-gray-800 font-medium">{formatFecha(c.fecha_consulta)}</p>
                                  </div>
                                </div>
                                {c.motivo && (
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1">
                                      ...
                                      <p className="text-sm text-gray-700 leading-relaxed bg-gray-50 p-3 rounded-lg">{c.motivo}</p>
                                    </div>
                                  </div>
                                )}
                              </div>
                ...
                <div className="p-16 text-center">
                  ...
                  <h3 className="text-2xl font-bold text-[#001F54] mb-3">No hay consultas programadas</h3>
                  <p className="text-gray-600 text-lg mb-6">No se encontraron citas médicas para la fecha seleccionada.</p>
                  <button
          ...
          {!fecha && !loading && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-600 text-xl font-medium">Selecciona una fecha para ver las consultas programadas</p>
            </div>
          )}
        </div>
      </div>
...
export default ConsultasFiltradas;
